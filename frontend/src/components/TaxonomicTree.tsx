...
import { useTaxonomicTree } from "../store/taxonomicTree";
...
export function TaxonomicTree(): JSX.Element {
  ...
  // ancestor-of-currently-selected set.
  const { visibleRows } = useMemo(() => {
    ...
    function walk(
      ...
      for (const row of kids) {
        ...
        rows.push({ row, depth, selectedAncestorIds: newAncestors });
  ...
  useEffect(() => {
    if (focusedId === null && visibleRows.length > 0) {
      ...
      if (first !== undefined) setFocusedId(first.row.id);
    }
  ...
  const onKeyDown = (
    ...
    const idx = visibleRows.findIndex((v) => v.row.id === id);
    if (idx === -1) return;
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = visibleRows[Math.min(idx + 1, visibleRows.length - 1)];
      if (next !== undefined) setFocusedId(next.row.id);
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      const prev = visibleRows[Math.max(idx - 1, 0)];
      if (prev !== undefined) setFocusedId(prev.row.id);
    } else if (e.key === "ArrowRight") {
      const row = visibleRows[idx]?.row;
      ...
      if (last !== undefined) setFocusedId(last.row.id);
  ...
  const registerRow = (id: number, el: HTMLButtonElement | null): void => {
    if (el === null) {
      ...
      <div className="mt-4">
        {roots === undefined && rootErr === undefined ? (
          ...
          <div
            ...
            aria-label="Taxonomic tree"
          >
            {visibleRows.map(({ row, depth, selectedAncestorIds }) => (
              <TreeRow
                key={row.id}
                row={row}
                depth={depth}
